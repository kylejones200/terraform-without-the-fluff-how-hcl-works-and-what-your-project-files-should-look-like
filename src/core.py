...
def analyze_terraform_structure(terraform_dir: Path) -> dict:
    ...
    return {'total_files': len(files), 'variable_files': len(vars_files), 'file_names': [f.name for f in files]}
...
def validate_terraform_structure(structure: dict) -> dict:
    ...
    return {'has_main': 'main.tf' in found_files, 'has_variables': 'variables.tf' in found_files, 'has_outputs': 'outputs.tf' in found_files, 'is_valid': all((req in found_files for req in required_files))}
...
def plot_terraform_structure(structure: dict, validation: dict, title: str, output_path: Path, plot: bool=False):
    ...
    values = [structure['total_files'], structure['variable_files'], 1 if validation['has_main'] else 0, 1 if validation['has_variables'] else 0, 1 if validation['has_outputs'] else 0]
